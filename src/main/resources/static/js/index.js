...
async function loadBooks(keyword="") {
    let url = "http://localhost:8080/book/list";
    if(keyword) url = `http://localhost:8080/book/search?keyword=${encodeURIComponent(keyword)}`;
    ...
    // 给按钮绑定事件
    document.querySelectorAll(".bookCard button").forEach(btn => {
...
async function addFavorite(bookId){
    ...
    try {
        const res = await fetch(`http://localhost:8080/favorite/add?userId=${userId}&bookId=${bookId}`, {
            method: "POST"
        });
